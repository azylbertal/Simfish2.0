...
def get_actor_core(
    ...
    bias_probability: Optional[float] = 0.0,
) -> R2D2Policy:
  ...
  def select_action(params: networks_lib.Params,
    ...
    # if state.is_biased:
      # If biased, choose greed action with probability (1-epsilon) and action number 3 with probability epsilon.
    #bias_action = jnp.array([3], dtype=networks_lib.Action.dtype)
    biased_greedy = jax.lax.cond(
        jax.random.uniform(policy_rng) < state.epsilon,
        lambda _: 3,  # Choose action 3 with probability epsilon
        lambda _: jnp.argmax(q_values, axis=-1),
        operand=None
    )
    action = jax.lax.cond(
        state.is_biased,
        lambda _: biased_greedy,
        lambda _: rlax.epsilon_greedy(state.epsilon).sample(policy_rng, q_values),
            operand=None)
    # else:          
  ...
  def init(
    ...
    rng, epsilon_rng, state_rng, bias_rng = jax.random.split(rng, 4)
    if num_epsilons:
      epsilon = jax.random.choice(epsilon_rng,
                                  np.logspace(0.5, 3, num_epsilons, base=0.1))
    else:
      epsilon = evaluation_epsilon
    is_biased = bias_probability > 0.0 and jax.random.uniform(
        bias_rng) < bias_probability
